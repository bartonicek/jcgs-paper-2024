...
#| echo: false
#| fig-align: "center"
#| fig-cap: "Examples of three methods for dealing with bars that have split into child components (segments). Top row: dodging (left) and stacking/highlighting (right); both by William Playfair (1801, 1822). Bottom row: layering (created with ggplot2; Wickham, 2010)."
#| message: false
#| warning: false
...
#| `ggplot2` default for handling empty subcategories). Data source: @henderson1981."
...
Stacked bars may offer several other advantages over dodged bars within the context of linked selection. First, we propose that stacking also provides a simpler mental model for *selection*. Since each stacked bar is a single visual entity, it makes intuitive sense that clicking it will select the entire category corresponding to it. Indeed, this is how many interactive data visualization systems implement linked selection: clicking on part of an object selects the category corresponding to the entire object [see e.g. @cook2023; @vegalite2022; @vanderplas2018; @xie2014; @theus2002; @urbanek2003; @urbanek2011; @swayne2003]. On the other hand, with dodged bars, the expectation is less clear: when we click on a segment in a dodged barplot, do we select the entire category or just the cases corresponding to the segment? Second, in a stacked barplot, the bar width and inter-bar gap are constant throughout selection. In a dodged barplot, it is not clear how to handle the gaps when some selection groups are empty - should we leave gaps for the (currently absent) highlighted segments, or draw wide bars and only shrink them in response to selection? If we do the former, the barplot may show asymmetric gaps and illusory clustering due to the absence of some segments. If we do the latter, we break the principle that the area of the bar should be proportional to the number of cases it represents [@cleveland1985]. Finally, by presenting fewer visual entities on the screen, stacked bars may also reduce cognitive load [@sweller2019; for a non-technical overview, see @knaflic2015].

To summarize, stacked bars exhibit a more graceful and visually pleasing behavior when combined with selection. In contrast, dodging produces non-uniform objects with jagged outline that changes significantly with selection. These difference may explain why stacking is still popular in the interactive data visualization community.
...
#| fig-show: "hold"
#| fig-align: "center"
#| fig-cap: "A representation is not the thing itself. Left: The Treachery of Images by René Magritte (1929) makes the point that a painting of a pipe is not the thing it represents (a pipe). Right: similarly, an filled with coloured rectangles is not a barplot, unless the rectangles accurately represent some underlying data."
...
#| echo: false
#| fig-height: 2.5
#| fig-cap: "\\label{fig:convexhull}Convex hull can be used as a valid representation of single-group selection but not multi-group selection. The light polygons shows the convex hull of the whole dataset, the dark polygon shows the hull of one selection group, the white dashed line shows the hull of a second selection group, and the medium-brightness polygon shows the combined hull of two selection groups. Points show individual cases. Left: since convex hull is a monoid, we can use it to show single highlighted group. Right: because of the lack of an inverse, we cannot guarantee that we will be able to recover either of the original hulls from the combined hull - we do not know how many interior points were \"forgotten\". The source of the data is the Palmer penguins data set, Horst 2020." 
#| message: false
#| warning: false
