...
combine these parts back together (such as linked selection), then we
need the statistics in our plots to fulfill certain algebraic
properties. Ultimately, understanding this hidden structure may empower
data visualization experts to build more flexible and expressive
...
\caption{Examples of three methods for dealing with bars that have split into child components (segments). Top row: dodging (left) and stacking/highlighting (right); both by William Playfair (1801, 1822). Bottom row: layering (created with ggplot2; Wickham, 2010).}\label{fig:playfair}
\end{figure}
...
for handling empty subcategories). Data source: \citet{henderson1981}.}
\end{figure}
...
On the other hand, with dodged bars, the expectation is less clear: when
we click on a segment in a dodged barplot, do we select the entire
category or just the cases corresponding to the segment? Second, in a
stacked barplot, the bar width and inter-bar gap are constant throughout
selection. In a dodged barplot, it is not clear how to handle the gaps
when some selection groups are empty - should we leave gaps for the
(currently absent) highlighted segments, or draw wide bars and only
shrink them in response to selection? If we do the former, the barplot
may show asymmetric gaps and illusory clustering due to the absence of
some segments. If we do the latter, we break the principle that the area
of the bar should be proportional to the number of cases it represents
...
To summarize, stacked bars exhibit a more graceful and visually pleasing
behavior when combined with selection. In contrast, dodging produces
non-uniform objects with jagged outline that changes significantly with
selection. These difference may explain why stacking is still popular in
the interactive data visualization community.
...
\caption{A representation is not the thing itself. Left: The Treachery of Images by René Magritte (1929) makes the point that a painting of a pipe is not the thing it represents (a pipe). Right: similarly, an filled with coloured rectangles is not a barplot, unless the rectangles accurately represent some underlying data.}\label{fig:magritte}
\end{figure}
...
thought of as a summary statistic, the convex hull \citep[see
e.g.][]{barber1996} provides a way to reduce a set of data points into a
simpler ``summary'', see Figure \ref{fig:convexhull}. We can recast
convex hull algorithm as a monoid: starting with an empty set, we
iterate through the data, adding points if they lie outside of the
current hull (and removing any points in the interior of the new hull).
This may not be the most computationally efficient method of finding
convex hulls, however, since it does meet the definition of a monoid,
this tells us \emph{what convex hulls can be used for}. If we combine
two convex hulls, we know that the new hull will be a valid summary for
the union of the underlying data points. In turn, this allows us to
...
polygon shows the combined hull of two selection groups. Points show
individual cases. Left: since convex hull is a monoid, we can use it to
show single highlighted group. Right: because of the lack of an inverse,
we cannot guarantee that we will be able to recover either of the
original hulls from the combined hull - we do not know how many interior
points were ``forgotten''. The source of the data is the Palmer penguins
data set, Horst 2020.}
\end{figure}
